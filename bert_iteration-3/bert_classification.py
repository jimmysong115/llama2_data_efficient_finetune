...
# 加载 positive 和 negative 数据
with open('/gluster_pdc_llm/user/songjielin/extracted_120k_data/better_data_combined/all_selected_output_iteration_0+1+2+3_second_better.json', 'r') as f:
    positive_data = json.load(f)
...
class_weights = torch.tensor([1.0,10.0]).cuda()
...
tokenizer = BertTokenizer.from_pretrained('/gluster_pdc_llm/user/songjielin/extracted_120k_data/bert_iteration_2/results/second/checkpoint-300')
...
trainer = WeightedLossTrainer(
    model=BertForSequenceClassification.from_pretrained('/gluster_pdc_llm/user/songjielin/extracted_120k_data/bert_iteration_2/results/second/checkpoint-300', num_labels=2),
    args=training_args,
